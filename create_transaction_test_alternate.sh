...
echo "Creating transaction"
sbt "run simpletransaction create --from-party self --from-contract default --to-party $TO_PARTY --to-contract $TO_CONTRACT -w test --genus-port 9091 -o $TX1_FILE -n private -a 100 -h localhost --keyfile $MAIN_KEY --walletdb $WALLET"
