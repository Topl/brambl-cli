...
```bash
brambl-cli wallet balance --from-party $PARTY --from-contract $CONTRACT --walletdb $WALLET --host $HOST --port $PORT
```

This will show you the balance of the address `$PARTY` and `$CONTRACT` in the wallet `$WALLET` in the node `$HOST:$PORT`. The `--from-state` can optionally be used to choose the state. The output will look something like this:
